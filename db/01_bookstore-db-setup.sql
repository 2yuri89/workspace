...
/* 보안. 어떤 컴퓨터에서 접속하는지. localhost(127.0.0.1)(서버가 있는 컴퓨터에서만 들어와)에서 humanda게정으로만 들어오면 쓸 수 있어 */
/* %는 와일드카드 어디서든 ㅇㅋ임. 그래서 사실 위에꺼만 하고 아래 %는 없어도 되긴 함 ㅇㅇ */

GRANT ALL PRIVILEGES ON bookstore.* to humanda@localhost;
GRANT ALL PRIVILEGES ON bookstore.* to humanda@"%";
/* 그란트 권한 주기. 북스토어 db에서 *(와일드 카드) = 해당 db에서는 다 해도 되. */

FLUSH PRIVILEGES;
/* 부여한 권한 당장 쓸 수 있도록하라고 새로고침 하는 느낌 */
