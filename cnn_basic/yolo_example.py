...
def load_yolo():
    ...
    model = cv2.dnn.readNet("yolo_data/yolov3.weights", "yolo_data/yolov3.cfg")
    ...
    colors = np.random.uniform(0, 255, size=(len(classes), 3)) # 80 * 3 -> 80개의

    return model, classes, layer_names, colors

def load_image(image_path):     # 샘플 돌리기위한 함수
...
def get_box_dimensions(outputs, height, width):     # get_box_dimensions: object 찾고 박스 찾기
	boxes = []
	confs = []
	class_ids = []
	for output in outputs:
		for detect in output:       # output 안에 여러 detect가 나올 것이다.
			scores = detect[5:]     # 앞의 4개는 디텍션된 오브젝트 중심이 어딘가임(사각형), 5 이후가 클래스별 신뢰도 점수
			class_id = np.argmax(scores) # 신뢰도 높은 object 찾기
			conf = scores[class_id]      # 신뢰도 점수
			if conf > 0.3:  # 80개 object 중 임계치를 0.3으로 줌. 젤 높게 평가한애가 0.3 이상이어야 믿을만 하다고 생각.(정하기 나름)
				center_x = int(detect[0] * width)   # 사각형 박스 정보 시작
				center_y = int(detect[1] * height)
				w = int(detect[2] * width)
				h = int(detect[3] * height)
				x = int(center_x - w/2)     # 좌측상단정보와 너비, 높이
				y = int(center_y - h / 2)
				boxes.append([x, y, w, h])
				confs.append(float(conf))   # 박스 정보 여기까지
...
if __name__ == "__main__":
    ...
    # # 2. load_image 테스트
    # image, height, width, channels = load_image("yolo_data/bicycle.jpg")
    ...
    # 4. 이미지 기반 개체 탐색
    # image_detect('yolo_data/busy_street.jpg')
	
    # 5. 동영상 기반 개체 탐색
    # video_detect("yolo_data/car_on_road.mp4")

    # 6. 실시간 영상 기반 개체 탐색
    webcam_detect() # webcam_detect에서 읽어오는 것이므로 경로 필요 없음
